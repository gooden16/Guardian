/*
  # Fix shift assignment counts

  1. Changes
    - Update trigger function to properly handle role assignments
    - Add additional validation for role quotas
...
-- Update the trigger function to handle role assignments properly
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- Get user's role from the assignment
    UPDATE shifts SET
      filled_slots = (
        SELECT COUNT(*) 
      ...
      filled_tl = (
        ...
        WHERE shift_id = NEW.shift_id 
        AND role = 'TL'
      ) + CASE WHEN NEW.role = 'TL' THEN 1 ELSE 0 END,
      filled_l1 = (
        SELECT COUNT(*) 
        FROM shift_assignments 
        WHERE shift_id = NEW.shift_id 
        AND role = 'L1'
      ) + CASE WHEN NEW.role = 'L1' THEN 1 ELSE 0 END,
      filled_l2 = (
        SELECT COUNT(*) 
        FROM shift_assignments 
        WHERE shift_id = NEW.shift_id 
        AND role = 'L2'
      ) + CASE WHEN NEW.role = 'L2' THEN 1 ELSE 0 END
  ...
  ELSIF TG_OP = 'DELETE' THEN
    ...
    UPDATE shifts SET
      ...
      filled_tl = (
        ...
        WHERE shift_id = OLD.shift_id 
        AND role = 'TL'
      ) - CASE WHEN OLD.role = 'TL' THEN 1 ELSE 0 END,
      filled_l1 = (
        SELECT COUNT(*) 
        FROM shift_assignments 
        WHERE shift_id = OLD.shift_id 
        AND role = 'L1'
      ) - CASE WHEN OLD.role = 'L1' THEN 1 ELSE 0 END,
      filled_l2 = (
        SELECT COUNT(*) 
        FROM shift_assignments 
        WHERE shift_id = OLD.shift_id 
        AND role = 'L2'
      ) - CASE WHEN OLD.role = 'L2' THEN 1 ELSE 0 END
    WHERE id = OLD.shift_id;
